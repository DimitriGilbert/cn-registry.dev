...
export default function CreatorProfilePage({ params }: PageProps) {
	...
	const githubMatch = creator.bio?.match(
	...
	const cleanBio = creator.bio
		?.replace(/GitHub:\s*https?:\/\/github\.com\/[^\s]+/g, "")
		.trim();
			...
			<div className="py-8">
				...
				<div className="relative mb-12 overflow-hidden rounded-2xl border bg-card p-8 shadow-sm">
					...
					<div className="relative flex flex-col items-start gap-8 md:flex-row">
						...
						<div className="group relative">
							<Avatar className="h-32 w-32 shadow-lg ring-4 ring-background transition-transform duration-200 group-hover:scale-105">
								<AvatarImage
							...
							{/* Activity Indicator */}
							<div className="-bottom-1 -right-1 absolute h-6 w-6 animate-pulse rounded-full border-2 border-background bg-green-400 shadow-sm" />
						</div>
						...
						<div className="flex-1 space-y-6">
							...
							{cleanBio && (
								<div className="relative">
									<p className="rounded-lg border bg-card/50 p-4 text-lg leading-relaxed backdrop-blur-sm">
										{cleanBio}
									</p>
								</div>
							)}
							...
							<div className="flex flex-wrap gap-6 text-muted-foreground">
								{creator.location && (
									<div className="flex items-center gap-2 rounded-full border bg-card/50 px-3 py-1.5 backdrop-blur-sm">
							...
							<div className="flex flex-wrap gap-3">
								{creator.website && (
									<Button
										...
										size="sm"
										asChild
										className="bg-card/50 backdrop-blur-sm transition-all duration-200 hover:scale-105 hover:bg-card"
									>
										<Link
									...
									<div className="flex flex-wrap gap-2">
										{creator.specialties.map((specialty, index) => (
											<Badge
												key={specialty}
												variant="secondary"
												className="border-primary/20 bg-primary/10 text-primary transition-all duration-200 hover:bg-primary/20"
												style={{
				...
				<div className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
					<Card className="border shadow-sm transition-all duration-200 hover:shadow-md">
						<div className="absolute inset-0 bg-primary/2 opacity-0 transition-opacity duration-200 hover:opacity-100" />
						<CardContent className="relative p-6">
							...
							<div className="mb-1 font-bold text-3xl text-blue-600 dark:text-blue-400">
								{statsLoading ? (
					...
					<Card className="border shadow-sm transition-all duration-200 hover:shadow-md">
						<div className="absolute inset-0 bg-accent/2 opacity-0 transition-opacity duration-200 hover:opacity-100" />
						<CardContent className="relative p-6">
							<div className="mb-2 flex items-center justify-between">
								<div className="rounded-lg bg-accent/10 p-2">
									<Star className="h-6 w-6 fill-current text-accent-foreground" />
								</div>
							...
							<div className="mb-1 font-bold text-3xl text-yellow-600 dark:text-yellow-400">
								{statsLoading ? (
									<Skeleton className="h-8 w-16" />
								) : (
									stats?.totalStars || 0
					...
					<Card className="border shadow-sm transition-all duration-200 hover:shadow-md">
						<div className="absolute inset-0 bg-secondary/2 opacity-0 transition-opacity duration-200 hover:opacity-100" />
						<CardContent className="relative p-6">
							<div className="mb-2 flex items-center justify-between">
