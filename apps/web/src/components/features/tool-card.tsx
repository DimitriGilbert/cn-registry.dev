...
export function ToolCard({
	...
	return (
		<Card className={cn(
			...
			!disableHoverEffects && "transition-all duration-150 hover:shadow-lg hover:scale-[1.02] hover:-translate-y-1"
		)}>
			...
			<div className={cn(
				...
				!disableHoverEffects && "transition-opacity duration-75 group-hover:opacity-100"
			)} />
				...
				<CardHeader className="pb-4">
						...
						<div className="space-y-2 flex-1">
							<div className="flex items-center gap-2">
								<CardTitle className={cn(
									...
									!disableHoverEffects && "transition-colors group-hover:text-accent"
								)}>
								...
								<div className={cn(
									...
									!disableHoverEffects && "group-hover:opacity-100 transition-opacity"
								)} />
							</div>
					...
					<div className="flex items-center gap-4 pt-2">
						<div className="flex items-center gap-3 text-xs text-muted-foreground">
							...
							{downloads && downloads > 0 && (
								<div className="flex items-center gap-1 bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-full">
					...
					<div className="flex items-center justify-between pt-3">
							...
							{categories
								...
								.map((category) => (
									<Badge 
										key={category.id} 
										variant="secondary" 
										className="text-xs px-2 py-0.5 bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20 hover:bg-orange-500/20 transition-colors"
									>
										{category.name}
									</Badge>
								))}
						...
						{creator && creator.username && (
							...
							>
								<Avatar className="h-5 w-5 ring-2 ring-transparent group-hover/creator:ring-orange-400/30 transition-all">
								...
								<span className="hidden sm:inline font-medium">{creator.name || creator.username}</span>
							</Link>
						)}
					</div>
				</CardHeader>
					...
					<div className="flex items-center gap-2">
						...
						{finalGithubUrl && (
							<Button 
								...
								className={cn(
									"hover:scale-[1.02]",
									!disableHoverEffects && "transition-transform duration-75 hover:border-accent"
								)}
							>
						...
						{websiteUrl && (
							<Button 
								...
								className={cn(
									"hover:scale-[1.02]",
									!disableHoverEffects && "transition-transform duration-75 hover:border-secondary"
								)}
							>
