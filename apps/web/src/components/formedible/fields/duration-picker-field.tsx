...
export const DurationPickerField: React.FC<DurationPickerFieldProps> = ({
	...
	const handleManualInput = (input: string) => {
		...
		const newHours = hourMatch
			? Math.min(Math.max(0, Number.parseInt(hourMatch[1], 10)), maxHours)
			: 0;
		const newMinutes = minuteMatch
			? Math.min(Math.max(0, Number.parseInt(minuteMatch[1], 10)), maxMinutes)
			: 0;
		const newSeconds = secondMatch
			? Math.min(Math.max(0, Number.parseInt(secondMatch[1], 10)), maxSeconds)
			: 0;
