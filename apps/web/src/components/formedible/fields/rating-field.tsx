...
export const RatingField: React.FC<RatingFieldSpecificProps> = ({
	...
	const getRatingValue = (index: number, isHalf = false): number => {
		return isHalf ? index + 0.5 : index + 1;
	};

	const shouldShowFilled = (index: number, isHalf = false): boolean => {
				...
				<div className="flex items-center gap-1">
					...
					{value > 0 && (
						<button
							type="button"
							className={cn(
								"ml-2 text-muted-foreground text-xs transition-colors hover:text-foreground",
