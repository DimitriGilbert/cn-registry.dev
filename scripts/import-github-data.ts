...
import { components, githubCache, tools } from "../apps/server/src/db/schema";
...
async function importGitHubDataForComponents(): Promise<void> {
	...
	const githubComponents = componentsWithGitHub.filter(
		(component) =>
			component.repoUrl && component.repoUrl.includes("github.com"),
	);

	console.log(
		`📦 Found ${githubComponents.length} components with GitHub URLs`,
	);
	...
	for (const component of githubComponents) {
		try {
			...
			if (!repoUrl) {
				console.log(
					`⚠️  Invalid GitHub URL for ${component.name}: ${component.repoUrl}`,
				);
				errorCount++;
				continue;
			}
			...
			console.log(
				`✅ Updated GitHub data for ${component.name} (⭐ ${githubData.stargazers_count})`,
			);
			successCount++;

			// Rate limiting - GitHub allows 60 requests per hour without token, 5000 with token
			await new Promise((resolve) =>
				setTimeout(resolve, process.env.GITHUB_TOKEN ? 1500 : 3000),
			);
		} catch (error) {
			console.error(
				`❌ Failed to fetch GitHub data for ${component.name}:`,
				error,
			);
			errorCount++;
		}
...
async function importGitHubDataForTools(): Promise<void> {
	...
	const githubTools = toolsWithGitHub.filter(
		(tool) => tool.repoUrl && tool.repoUrl.includes("github.com"),
	);
	...
	for (const tool of githubTools) {
		try {
			...
			console.log(
				`✅ Updated GitHub data for ${tool.name} (⭐ ${githubData.stargazers_count})`,
			);
			successCount++;
...
async function main() {
	...
	if (!process.env.GITHUB_TOKEN) {
		console.log(
			"⚠️  No GITHUB_TOKEN found. Rate limiting will be more restrictive.",
		);
		console.log(
			"   Add GITHUB_TOKEN to your .env file for better performance.",
		);
	}
